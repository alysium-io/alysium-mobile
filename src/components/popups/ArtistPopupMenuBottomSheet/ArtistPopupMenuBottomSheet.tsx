...
interface ArtistPopupMenuBottomSheetProps {
	sheetApi: SheetApi;
	artist_uid: NanoId;
}

const ArtistPopupMenuBottomSheet: React.FC<ArtistPopupMenuBottomSheetProps> = ({
	sheetApi,
	artist_uid
}) => {
		...
		<BottomSheet sheetRef={sheetApi.sheetRef} enableDynamicSizing>
			<BottomSheetView style={{ flex: 1, paddingBottom: insets.bottom + 25 }}>
				<View margin='m' alignItems='center'>
					<QRCode value={UNIVERSAL_LINK_PREFIX + '/artist/' + artist_uid} />
				</View>
				<MenuListItem
					titleTextProps={{
						title: 'Share',
...
export default ArtistPopupMenuBottomSheet;
